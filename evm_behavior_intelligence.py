...
def inspect_behavior(market: MarketData) -> BehaviorReport:
    signals=[]; warnings=[]; score=50
    if market.sells_5m > market.buys_5m * 2 and market.sells_5m >= 10:
        signals.append("ACUTE_SELL_PRESSURE"); score -= 20
    elif market.buys_5m > market.sells_5m * 2 and market.buys_5m >= 10:
        signals.append("ACUTE_BUY_PRESSURE"); score += 15
    if market.sells_1h > market.buys_1h * 2 and market.sells_1h >= 20:
        signals.append("SUSTAINED_SELL_PRESSURE"); score -= 20
    if market.volume_5m > 0 and market.liquidity_usd > 0 and market.volume_5m / market.liquidity_usd > 1:
